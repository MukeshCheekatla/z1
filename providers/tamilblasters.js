/**
 * TamilBlasters - Elite [W] Search Edition
 * Powerhouse for South Indian content
 */
...
function getStreams(tmdbId, type, season, episode) {
    ...
    return fetch(tmdbUrl)
        ...
        .then(function(mediaInfo) {
            ...
            var searchUrl = MAIN_URL + "/index.php?app=core&module=search&do=search&q=" + encodeURIComponent(query) + "&quicksearch=1";
            ...
            return fetch(searchUrl, { headers: HEADERS })
                ...
                .then(function(html) {
                    ...
                    $("a").each(function() {
                        ...
                        // TamilBlasters often uses [W] or [Watch]
                        if ((text === "[W]" || text === "[WATCH]") && href) {
                            var parent = $(this).closest("li, div, p");
                            var topicTitle = parent.find('a[href*="/forums/topic/"]').first().text().trim() || title;
                            watchLinks.push({ t: topicTitle, u: href });
                        }
                    });

                    // Fallback to homepage scouting
                    if (watchLinks.length === 0) {
                        return fetch(MAIN_URL, { headers: HEADERS })
                            .then(function(r) { return r.text(); })
                            .then(function(homeHtml) {
                                var $h = cheerio.load(homeHtml);
                                $h('a:contains("[WATCH]"), a:contains("[W]")').each(function() {
